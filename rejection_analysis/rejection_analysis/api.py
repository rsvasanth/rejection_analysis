...
def get_dashboard_metrics(date=None, inspection_type="Lot Inspection"):
    ...
    # Use hardcoded threshold for now
    threshold = 5.0
...
def get_lot_inspection_report(filters=None):
    ...
    # Simplified SQL query without Job Card dependency
    query = """
        SELECT
    ...
    results = []
    for row in data:
        # Use moulding date if available, else inspection posting date
        production_date = row.get("moulding_date") or row.get("posting_date")

        # Use employee_name from production if available, else operator_name from inspection
        operator_name = row.get("employee_name") or row.get("operator_name")

        result = {
            "inspection_entry": row.get("inspection_entry"),
            "production_date": str(production_date) if production_date else None,
            "shift_type": None,  # Will add shift_type field later
